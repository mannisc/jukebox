...
EndIf
;}
Global starttime = gettickcount_()
...
portals(0) = "youtube.dll"
portals(1) = "muzu.dll"
portals(2) = "dailymotion.dll"
portals(3) = "metacafe.dll"
portals(4) = "googlevideo.dll"
  ...
  ElseIf *Request\type =2  
    ...
    *VideoSide.VideoSide = *vsidedim(s)
    For vsideindex = 0 To NumVSides
      ...
      Protected VideoSide.VideoSide
      VideoSide\name.s    = *VideoSide\name
      VideoSide\dllName   = *VideoSide\dllName
      VideoSide\dllVersion= *VideoSide\dllVersion
      VideoSide\dllID     = *VideoSide\dllID
      VideoSide\message          = AllocateMemory(100002)
      VideoSide\messageReady     = 1
      VideoSide\kill             = AllocateMemory(1000002)
      VideoSide\suchen           = AllocateMemory(10002)
      VideoSide\suchenEX         = AllocateMemory(10002)
      VideoSide\_GetPageSearch   = *VideoSide\_GetPageSearch
      VideoSide\_GetPageSearchEx = *VideoSide\_GetPageSearchEx
      VideoSide\_IsURLVideo      = *VideoSide\_IsURLVideo
      VideoSide\_GetMovieFLV     = *VideoSide\_GetMovieFLV
      VideoSide\searchthread     = 0
      VideoSide\searched         = 0
      VideoSide\showstate        = 0
      VideoSide\VideoID          = *VideoSide\VideoID
      VideoSide\updateVideos     = 0
      VideoSide\pauseThread      = 0
      VideoSide\dllVersion       = *VideoSide\dllVersion
      VideoSide\updateVideos = 0
      ...
      Debug "REPEAT"
      SetGadgetText(egadget,GetGadgetText(egadget)+"Server wartet auf Suchergebnisse: "+*Request\params+Chr(10))
      message.s = ""
      Repeat 
        If message = ""
          If VideoSide\messageReady = 0
            ...
            If  message <> "" 
              Debug "-----------"
              Debug message
              SetGadgetText(egadget,GetGadgetText(egadget)+"Suchergebnis erhalten! "+Chr(10))
              Debug "messege received!"
              oldkill.s = PeekS(VideoSide\kill)
              PokeS(VideoSide\kill,oldkill+Chr(10)+Str(VideoSide\currentsearchthreadID)+Chr(10))
              VideoSide\newkill +1
            EndIf
          EndIf 
        EndIf 
        ...
        EndIf
        Delay(1)
      Until  VideoSide\messageReady = -1 
      SetGadgetText(egadget,GetGadgetText(egadget)+"Suche abgechlossen! "+Chr(10))
...
Procedure ClientThread(*Client.Client)
  Debug *Client
  NewList requests.request()
  Repeat
    If *Client\availableinput
      SetGadgetText(egadget,GetGadgetText(egadget)+"Anfrage wird beartbeitet!..."+Chr(10))
      *Client\readmem = 1
      inputstring.s = PeekS(*Client\inputbuffer,-1,#PB_Ascii)
      SetGadgetText(egadget,GetGadgetText(egadget)+inputstring+Chr(10))
      Debug inputstring
      ...
      *Client\readmem       = 0
      ...
      If StringField(inputstring,1," ") = "GET"
        ...
        SetGadgetText(egadget,GetGadgetText(egadget)+"PARAMS: "+ params+Chr(10))
        If url 
...
; FoldLines=0009002D0031003A003C004900A000A800E2011701D80232
; Build=0
